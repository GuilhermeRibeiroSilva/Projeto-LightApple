document.addEventListener("DOMContentLoaded", function() {
  ...
  async function carregarAnuncios(pagina = 1) {
      try {
          const response = await fetch(`carregar_estabelecimentos_coleta.php?pagina=${pagina}`);
          ...
          if (data.locais && data.locais.length > 0) {
              data.locais.forEach(local => {
                  ...
                  div.innerHTML = `
                      ...
                      <h3>${local.nome}</h3>
                      <p class="distancia">Distância: ${local.distancia || 'Calculando...'} km</p>
                  `;

                  productsGrid.appendChild(div);
              });

              // Tenta obter a localização do usuário para atualizar as distâncias
  ...
  function atualizarPaginacao(paginacao) {
      ...
      html += `<button class="prev" ${pagina_atual <= 1 ? 'disabled' : ''} 
               onclick="window.carregarAnuncios(${pagina_atual - 1})">Anterior</button>`;

      // Números das páginas
      for (let i = 1; i <= total_paginas; i++) {
          html += `<button class="page-number ${i === pagina_atual ? 'active' : ''}" 
                   onclick="window.carregarAnuncios(${i})">${i}</button>`;
      }

      // Botão Próximo
      html += `<button class="next" ${pagina_atual >= total_paginas ? 'disabled' : ''} 
               onclick="window.carregarAnuncios(${pagina_atual + 1})">Próximo</button>`;
